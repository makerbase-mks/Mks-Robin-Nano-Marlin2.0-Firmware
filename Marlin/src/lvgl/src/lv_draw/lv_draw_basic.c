    ...
    if(chroma_key == false && alpha_byte == false && opa == LV_OPA_COVER && recolor_opa == LV_OPA_TRANSP) {
        ...
        /*Normal native VDB*/
			...
			if(cur_pic.is_gcode == 1)
			{
				...
				{
					lv_open_gcode_file(&cur_pic.name[0]);
			...
			else
			{
				...
				else
				{
					...
					if(strcmp(&cur_pic.name[0],&cur_pic.name_last[0])!=0)
					{
				...
				strcpy(&cur_pic.name_last[0],&cur_pic.name[0]);
            ...
            for(row = masked_a.y1; row <= masked_a.y2; row++) {
				...
				if(cur_pic.is_gcode == 1)
				{
					lv_gcode_file_read(vdb_buf_tmp);
					lv_pic_addr_offset +=map_width * px_size_byte;
					
				}
				else
				{
					lv_pic_test(vdb_buf_tmp,cur_pic.addr+lv_pic_addr_offset,map_width * px_size_byte);
					//memcpy(vdb_buf_tmp, bmp_public_buf,map_useful_w * sizeof(lv_color_t));
					lv_pic_addr_offset +=map_width * px_size_byte;
				}
#endif

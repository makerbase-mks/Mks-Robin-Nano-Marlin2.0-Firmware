...
void lv_draw_wifi_list(void)
{
	...
	lv_imgbtn_set_style(buttonDown, LV_BTN_STATE_PR, &tft_style_lable_pre);
	lv_imgbtn_set_style(buttonDown, LV_BTN_STATE_REL, &tft_style_lable_rel);
	...
	for(uint8_t i = 0; i < NUMBER_OF_PAGE; i++)
	{
		...
		lv_btn_set_style(buttonWifiN[i], LV_BTN_STYLE_REL, &tft_style_lable_rel);    /*Set the button's released style*/
		lv_btn_set_style(buttonWifiN[i], LV_BTN_STYLE_PR, &tft_style_lable_pre);      /*Set the button's pressed style*/
	...
	lv_obj_set_style(lablePageText, &tft_style_lable_rel);
...
void disp_wifi_list(void)
{
	...
	{	
		...
		else
		{
			...
			{
				lv_btn_set_style(buttonWifiN[i], LV_BTN_STYLE_REL, &tft_style_lable_rel);
			}
		}
	}
}

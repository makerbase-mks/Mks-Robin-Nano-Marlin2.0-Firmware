			...
			switch(cmd_value)
			{
				...
				case 994:
					if((uiCfg.print_state == WORKING) || (uiCfg.print_state == PAUSED))
					{
						...
						sprintf((char *)tempBuf, "M994 %s;%d\n", list_file.file_name[sel_id],gCfgItems.curFilesize);
						wifi_ret_ack();
						send_to_wifi((char *)tempBuf, strlen((char *)tempBuf));	
					}
					break;
				...
				case 115:
					memset(tempBuf,0,sizeof(tempBuf));
					send_to_wifi("ok\r\n", strlen("ok\r\n"));					
					send_to_wifi("FIRMWARE_NAME:Robin_nano\r\n", strlen("FIRMWARE_NAME:Robin_nano\r\n"));					
					break;
...
int storeRcvData(int32_t len)
{
	...
	{
		for(uint16_t i=0;i<len;i++)wifiDmaRcvFifo.bufferAddr[tmpW][i] = WIFISERIAL.read();

...
static void event_handler(lv_obj_t * obj, lv_event_t event) {
  ...
  void lv_draw_operation(void) {
    lv_obj_t *buttonSpeed = NULL, *label_Speed = NULL;
    ...
    if(gCfgItems.uiStyle == PRINT_STYLE) {
      lv_obj_t *buttonPreHeat = NULL, *buttonExtrusion = NULL;
      lv_obj_t *buttonBack = NULL, *buttonFan = NULL;
      lv_obj_t *labelPreHeat = NULL, *labelExtrusion = NULL;
      lv_obj_t *label_Back = NULL, *label_Fan = NULL;
      ...
      lv_imgbtn_set_style(buttonSpeed, LV_BTN_STATE_PR, &tft_style_label_pre);
      lv_imgbtn_set_style(buttonSpeed, LV_BTN_STATE_REL, &tft_style_label_rel);
      lv_obj_set_pos(buttonSpeed, INTERVAL_V, titleHeight);
      lv_btn_set_layout(buttonSpeed, LV_LAYOUT_OFF);
      label_Speed = lv_label_create(buttonSpeed, NULL);

      lv_obj_t *buttonCutTimes = lv_imgbtn_create(scr, NULL);
      lv_obj_set_event_cb_mks(buttonCutTimes, event_handler, ID_O_CUT_TIMES, NULL, 0);
      lv_imgbtn_set_src(buttonCutTimes, LV_BTN_STATE_REL, "F:/bmp_Cut_times.bin");
      lv_imgbtn_set_src(buttonCutTimes, LV_BTN_STATE_PR, "F:/bmp_Cut_times.bin");
      lv_imgbtn_set_style(buttonCutTimes, LV_BTN_STATE_PR, &tft_style_label_pre);
      lv_imgbtn_set_style(buttonCutTimes, LV_BTN_STATE_REL, &tft_style_label_rel);
      lv_obj_set_pos(buttonCutTimes, BTN_X_PIXEL + INTERVAL_V * 2, titleHeight);
      lv_btn_set_layout(buttonCutTimes, LV_LAYOUT_OFF);
      lv_obj_t *label_cutTimes = lv_label_create(buttonCutTimes, NULL);
      ...
      if (gCfgItems.multiple_language != 0) {
        ...
        lv_label_set_text(label_cutTimes, spindle_laser_menu.cutTimes);
        lv_obj_align(label_cutTimes, buttonCutTimes, LV_ALIGN_IN_BOTTOM_MID, 0, BUTTON_TEXT_Y_OFFSET);
        ...
        if (gCfgItems.encoder_enable == true) {
          lv_group_add_obj(g, buttonSpeed);
          lv_group_add_obj(g, buttonCutTimes);
          lv_group_add_obj(g, buttonBack);
        }
      #endif // BUTTONS_EXIST(EN1, EN2, ENC)
    }

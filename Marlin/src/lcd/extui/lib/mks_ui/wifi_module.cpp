			...
			switch(cmd_value) {
				...
				case 23:					
					...
					if(uiCfg.print_state == IDLE) {
						...
						if(strstr((char *)&tmpStr[index], ".g") || strstr((char *)&tmpStr[index], ".G") || strstr((char *)&tmpStr[index], ".n") || strstr((char *)&tmpStr[index], ".N")) {
							if(strlen((char *)&tmpStr[index]) < 80) {
				...
				case 998:
					...
					break;
				case 986:   //engrave times
					tmpStr = (uint8_t *)strstr((const char *)&cmd_line[0], "T");
					if(tmpStr) {
						cmd_value = atoi((const char *)(tmpStr + 1));
						NOMORE(cmd_value, 100);
						NOLESS(cmd_value, 0);
						uiCfg.cutTimes = cmd_value;
					}
					send_to_wifi((char *)"ok\r\n", strlen("ok\r\n"));
					break;
				case 115:
...
void stopEspTransfer() {
	...
	#if HAS_SERVOS
    	if(gCfgItems.uiStyle == PRINT_STYLE) servo_init();
  	#endif

  	#if HAS_Z_SERVO_PROBE
    	if(gCfgItems.uiStyle == PRINT_STYLE) probe.servo_probe_init();
  	#endif
